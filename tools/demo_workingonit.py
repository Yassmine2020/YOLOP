...
skip_factor = 20     # Skip every other frame for half FPS
frame_counter = 0
...
def detect(cfg,opt):
    ...
    for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
        if i % 3 != 0:  # Continue to next iteration if i is not a multiple of 5
            continue
...
##📛 TEST
        ...
        mask_det = extract_data(ll_predict_img)
        mask_det = cv2.resize(mask_det, (img_det.shape[1], img_det.shape[0]))
        img_det = cv2.addWeighted(img_det, 0.7, mask_det, 0.3, 0)

        print(f"img_det shape: {img_det.shape}, dtype: {img_det.dtype}")
        print(f"mask_det shape: {mask_det.shape}, dtype: {mask_det.dtype}")
...
##📛 TEST
 ...
 #         📛 Test
        ...
        #         fourcc = 'mp4v'  # output video codec
        #         fps = vid_cap.get(cv2.CAP_PROP_FPS) / skip_factor
        #         h,w,_=img_det.shape
        #         vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*fourcc), fps, (w, h))
                

        #             # Increment frame counter for every frame read
        #     frame_counter += 1
        #     print(frame_counter)
        #     # Check if the current frame is one that should be processed
        #     if frame_counter % skip_factor == 0:
        #         print('▶️ frame counter', frame_counter)
        #         vid_writer.write(img_det)
 #         📛 Test
